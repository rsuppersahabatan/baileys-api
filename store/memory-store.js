...
import { jidNormalizedUser, toNumber } from '@innovatorssoft/baileys'
import { EventEmitter } from 'events'

/** A JID that belongs to an individual user (as opposed to a group or a broadcast list). */
...
class ConcurrentStore extends EventEmitter {
    ...
    async smartAutoSave() {
        ...
        if (this.isProcessingHistory && this.syncStartTime && Date.now() - this.syncStartTime < 30000) {
            // First 30 seconds of sync
    ...
    hasBackupData() {
        return this.backupData.chats.size > 0 || this.backupData.messages.size > 0 || this.backupData.contacts.size > 0
    }
    ...
    async writeToFileInternal(file, data) {
        ...
        try {
            ...
            if (stats.size < 10) {
                // Less than 10 bytes indicates a virtually empty file
    ...
    async processMessagesOptimized(newMessages) {
        ...
        for (const [jid, chatMessages] of messagesByChat.entries()) {
            await this.processChatMessages(jid, chatMessages)
        }
    }
    ...
    async processChatMessages(jid, chatMessages) {
        ...
        if (existingMessages.size > this.config.maxMessagesPerChat) {
            const sortedMessages = Array.from(existingMessages.entries()).sort(
    ...
    updateStats() {
        ...
        this.stats.totalMessages = Array.from(this.messages.values()).reduce(
            (total, chatMsgs) => total + chatMsgs.size,
            0,
        )
    }
    ...
    async serializeStoreData() {
        return {
            ...
            messages: Object.fromEntries([...this.messages.entries()].map(([jid, msgs]) => [jid, [...msgs.entries()]])),
    ...
    /** Stop the auto-save timer without touching what is already on disk. */
    dispose() {
    ...
    getLastMessage(jid) {
        ...
        return msgsArray.sort((a, b) => (b.messageTimestamp || 0) - (a.messageTimestamp || 0))[0]
    }
    ...
    /**
     * A single stored message by id, or null when it is not cached.
     * The API uses this to resolve a message before forwarding or downloading it.
     */
    getMessage(jid, messageId) {
        if (!messageId) return null

        return this.messages.get(jidNormalizedUser(jid))?.get(messageId) ?? null
    }

    /**
     * Messages of a chat, newest first by default.
     *
     * `before` and `after` accept either a unix timestamp or a cursor object
     * (`{ id, fromMe }`). The cursor is resolved to the position of that message
     * so paginating never skips or repeats messages sharing a timestamp.
     */
    async loadMessages(jid, options = {}) {
        const { limit = 50, offset = 0, before = null, after = null, sortOrder = 'desc' } = options

        const chatMessages = this.messages.get(jidNormalizedUser(jid))
        ...
        msgsArray = applyCursor(msgsArray, before, 'before', sortOrder)
        msgsArray = applyCursor(msgsArray, after, 'after', sortOrder)

        return msgsArray.slice(offset, offset + limit)
    }
}

/** Index of the message a cursor points at, or -1 when it is not in the list. */
const cursorIndex = (messages, cursor) => {
    if (cursor === null || typeof cursor !== 'object') return -1

    return messages.findIndex(
        (msg) => msg.key?.id === cursor.id && (cursor.fromMe === undefined || msg.key?.fromMe === cursor.fromMe),
    )
}

/**
 * Keep only the messages on one side of a cursor.
 *
 * A number is read as a unix timestamp. An object is read as a message cursor
 * and resolved by position, which keeps messages sharing a timestamp together
 * instead of dropping them at a page boundary.
 */
const applyCursor = (messages, cursor, side, sortOrder) => {
    if (cursor === null || cursor === undefined) return messages

    const index = cursorIndex(messages, cursor)

    if (index >= 0) {
        // In descending order "before" means further down the list.
        const keepAfter = (side === 'before') === (sortOrder === 'desc')

        return keepAfter ? messages.slice(index + 1) : messages.slice(0, index)
    }

    if (typeof cursor !== 'number') return messages

    return messages.filter((msg) =>
        side === 'before' ? (msg.messageTimestamp || 0) < cursor : (msg.messageTimestamp || 0) > cursor,
    )
}
